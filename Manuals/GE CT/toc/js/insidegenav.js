function Folder(folderDescription, hreference,targetwin) //constructor 
{ 
  //constant data 
  this.target = 'S';
  if (targetwin == true)
  ...
  this.setState1 = setStateFolder1 
...
{ 
  ...
  if (browserVersion > 0) 
    auxEv = "<a href='javascript:clickOnNode1(\""+this.getID()+"\")' onMouseOver='winStatus(\" \"); return true;'>" 
  else 
    auxEv = "<a>" 
  ...
  if (this.level>0) 
  {
    if (this.isLastNode) //the last child in the children array 
    	if(nodeName != 'ftv2node.gif')
    	{
	    leftSide = leftSide + "<td  valign=bottom>" + auxEv + "<img name='nodeIcon" + this.id + "' id='nodeIcon" + this.id + "' src='" + nodeName + "' border=0></a></td>"
	}
	else
	    leftSide = leftSide + "<td valign=bottom>" + auxEv + "<img name='nodeIcon" + this.id + "' id='nodeIcon" + this.id + "' src='" + nodeName + "' width=1 height=12 border=0></a></td>"
    else 
    	if(nodeName != 'ftv2node.gif')
    	{
			  if (this.level==1) 
  				{
					if(this.nChildren == 0) {
						    leftSide = leftSide + "<td valign=bottom ><img name='nodeIcon" + this.id + "' id='nodeIcon" + this.id + "' src='"+ ICONPATH + "spacer.gif' width=18 height=12 border=2></a></td>"
							} else {
					    	leftSide = leftSide + "<td valign=bottom >" + auxEv + "<img name='nodeIcon" + this.id + "' id='nodeIcon" + this.id + "' src='" + nodeName + "' border=0></a></td>"
  ...
  docW = docW + "<tr>" + leftSide + "<td valign=bottom>";
  if (USEICONS)
  {
...
{ 
  ...
  if (this.getID()!=foldersTree.getID() && PERSERVESTATE && !this.isOpen) //closing
  {
     ...
     if (currentOpen != "favoriteCookie") {
         currentOpen = currentOpen.replace(this.getID()+cookieCutter, "")
         SetCookie("clickedFolder", currentOpen)
     }
     currentOpen1 = GetCookie("favoritesFolder")
     
     if (currentOpen1 != "favoriteCookie") {
         currentOpen1 = currentOpen1.replace(this.getID()+cookieCutter, "")
         //alert(currentOpen1);
         SetCookie("favoritesFolder", currentOpen1)
     }
     //alert("function setStateFolder(isOpen) "+GetCookie("favoritesFolder"));
  }
...
function setStateFolder1(isOpen) 
{ 
  var subEntries 
  var totalHeight 
  var fIt = 0 
...
{ 
  ...
  { 
	...
	{
	  if(this.hreference.indexOf("http://doNothing") >= 0 || this.hreference.indexOf("http://donothing") >= 0)
	  {
	  	  	docW = docW + "<a href='javascript:clickOnNode1(\""+this.getID()+"\")'"
	  }
/*	  else if(this.hreference.indexOf("http://") >= 0 || this.hreference.indexOf("https://") >= 0)
	  {
	  	docW = docW + "<a href='" + this.hreference + "' TARGET=_blank "
	  }	
*/	  else if(this.target== 'B')
	  {
	  	docW = docW + "<a href='" + this.hreference + "' TARGET=_blank "
	  }	
	  else
...
function nodeImageSrc() {
  ...
  { 
    if (this.nChildren == 0)
      ...
      if (this.isOpen)
        srcStr = ICONPATH + "primListOpen.gif"  
      else
        srcStr = ICONPATH + "primListClosed.gif"  
  } 
  else 
  { 
    if (this.nChildren == 0)
  ...
  if (srcStr == "") {
  	//alert('hello');
  	srcStr = ICONPATH + "subListClosedcarat.gif"
  } 
  return srcStr;
}
...
{ 
  ...
  {
   fullLink = "href=\""+this.link+"\" target=\""+this.target+"\" ";
  }
  else
	{
  ...
  if (this.level>0 && this.level<=2) 
    ...
    { 
      ...
      leftSide = leftSide + "<td valign=top><img src='" + ICONPATH + "ftv2lastnode.gif' width=1 height=12></td>"
    } 
    else 
    { 
      leftSide = leftSide + "<td valign=top ><img src='" + ICONPATH + "ftv2node.gif' width=1 height=14></td>"
    } 
  else if(this.level>2)
  	if (this.isLastNode) //the last 'brother' in the children array 
	{ 
	    leftSide = leftSide + "<td valign=top><img src='" + ICONPATH + "ftv2lastnode.gif' width=3 height=12></td>"
	    
	} 
	else 
	{ 
	   leftSide = leftSide + "<td valign=top ><img src='" + ICONPATH + "ftv2node.gif' width=3 height=12></td>"
  ...
  {
  	this.desc = this.desc.substring(this.desc.indexOf('-')+1)
  	if (USETEXTLINKS) 
  	docW = docW + "<img src='/static/images/spacer.gif' height='6' />- <a " + fullLink + " id=\"itemTextLink"+this.id+"\">" + this.desc + "</a>"
  	else 
  	  docW = docW + this.desc
  }else{
  if (USETEXTLINKS) 
...
function leftSideHTML(leftSideCoded) {
	...
	for (i=0; i<leftSideCoded.length; i++)
	{
		...
		{
			retStr = retStr + "<td valign=bottom ><img src='" + ICONPATH + "ftv2vertline.gif' width=16 height=12></td>"
		}
		if (leftSideCoded.charAt(i) == "0")
		{
			retStr = retStr + "<td valign=bottom><img src='" + ICONPATH + "ftv2blank.gif' width=16 height=12></td>"
		}
	}
	return retStr
}
...
{ //alert('Into Click');
  clickOnNodeObj1(findObj(folderId))
}

function clickOnNode1(folderId) 
{ //alert('Into Click');
...
{ 
  ...
  if (folderObj.id!=foldersTree.id && PERSERVESTATE)
  {
    ...
    if (!folderObj.isOpen) //closing
    {
      currentOpen1 = currentOpen1.replace(folderObj.getID()+cookieCutter, "")
      SetCookie("favoritesFolder", currentOpen1)
    }
    else
    {
      SetCookie("favoritesFolder", currentOpen1+folderObj.getID()+cookieCutter)
    }
   
  }
}
function clickOnNodeObj1(folderObj) 
{ 
  var state = 0 
  var currentOpen
 
  state = folderObj.isOpen 
  folderObj.setState1(!state) //open<->close  
	//alert('am called1')
  if (folderObj.id!=foldersTree.id && PERSERVESTATE)
  {
    currentOpen = GetCookie("clickedFolder")
...
function preLoadIcons() {
	...
	auxImg.src = ICONPATH + "primListOpen.gif";
	auxImg.src = ICONPATH + "primListOpen.gif";
	auxImg.src = ICONPATH + "primListClosed.gif";
	auxImg.src = ICONPATH + "primListClosed.gif";
...
function PersistentFolderOpening()
{
  ...
  //alert("Persist"+stateInCookie+" "+(stateInCookie!="favoriteCookie"));
  SetCookie('clickedFolder', "favoriteCookie") //at the end of function it will be back, minus null cases
  if (stateInCookie == null )
    stateInCookie = "favoriteCookie";
  
  if(stateInCookie!="favoriteCookie")
  {
    fldArr = stateInCookie.replace("favoriteCookie","").split(cookieCutter)
  ...
  stateInCookie1 = GetCookie("favoritesFolder");
  
  SetCookie('favoritesFolder', "favoriteCookie") //at the end of function it will be back, minus null cases
  
  if (stateInCookie1 == null )
    stateInCookie1 = "favoriteCookie";
    
  if(stateInCookie1!="favoriteCookie")
  {
    fldArr = stateInCookie1.replace("favoriteCookie","").split(cookieCutter)
    for (fldPos=0; fldPos<fldArr.length; fldPos++)
    {
      fldStr=fldArr[fldPos]
      if (fldStr != "") {
...
function storeAllNodesInClickCookie(treeNodeObj)
{
  ...
  if (typeof treeNodeObj.setState != "undefined") //is folder
  {
    ...
    if (treeNodeObj.getID() != foldersTree.getID()){
      ...
      SetCookie("favoritesFolder", currentOpen+treeNodeObj.getID()+cookieCutter)
	}
...
{ 
  name = CookieBranding(name);
  if (name == "favoritesFolder"){
      DelCookie ("favoritesFolder","/",".ge.com") ;
  }
...
{ 
  ...
  //alert(GetCookie('favoritesFolder'));
  setInitialLayout()
...
// WINDOW STATUS FUNCTIONS. DISPLAYS LINK TITLE/NAME IN BROWSER WINDOW
showWinStatus = true;
window.defaultStatus = "";
function winStatus(msg) {
	if (showWinStatus) window.status = msg;
} 
