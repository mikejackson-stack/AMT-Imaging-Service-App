...
function makeDateFilter()
{
    ...
    var begin, end;
    var beginDay, endDay, beginMonth, endMonth

	if (isFieldBlank(document.SearchForm.StartDay))
		beginDay = "D01";
	else
		beginDay = document.SearchForm.StartDay.value;


	if (isFieldBlank(document.SearchForm.EndDay))
		endDay = "D31";
	else
    ...
    if (isFieldBlank(document.SearchForm.StartYear))
        ...
        begin =  beginMonth + "/" + beginDay + "/" + document.SearchForm.StartYear.value;
    if (isFieldBlank(document.SearchForm.EndYear))
        end = "M12/D31/Y2999";
    else
        end =  endMonth + "/" + endDay + "/" + document.SearchForm.EndYear.value;
...
function clearDateSearch() {
	clearSelectControl(document.SearchForm.StartMonth);
	clearSelectControl(document.SearchForm.EndMonth);
	clearSelectControl(document.SearchForm.StartDay);
	clearSelectControl(document.SearchForm.EndDay);
	clearSelectControl(document.SearchForm.StartYear);
	clearSelectControl(document.SearchForm.EndYear);
    updateDateFilter()
    }
    
function clearSelectControl(ctrl)
{	if (!ctrl)
		return;
